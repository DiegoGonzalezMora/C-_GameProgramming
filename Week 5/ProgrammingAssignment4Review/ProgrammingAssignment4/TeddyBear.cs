...
namespace ProgrammingAssignment4
{
    ...
    {
        #region Fields        // collecting support
        bool collecting = false;
        bool targetSet = false;        // drawing support
        Texture2D sprite;
        Rectangle drawRectangle;
        int halfDrawRectangleWidth;
        int halfDrawRectangleHeight;        // moving support
        const float BaseSpeed = 0.3f;
        Vector2 location;
        Vector2 velocity = Vector2.Zero;        // click processing
        ...
        {
            this.sprite = sprite;
            this.location = location;            // STUDENTS: set draw rectangle so teddy is centered on location
            drawRectangle = new Rectangle((int)location.X - sprite.Width / 2,
                (int)location.Y - sprite.Height / 2, sprite.Width, sprite.Height);            // STUDENTS: set halfDrawRectangleWidth and halfDrawRectangleHeight for efficiency
        ...
        public void Update(GameTime gameTime, MouseState mouse)
        {
            ...
            if (Collecting == true)
            {
                location.X = location.X + (velocity.X * (float)gameTime.ElapsedGameTime.TotalSeconds);
                location.Y = location.Y + (velocity.Y * (float)gameTime.ElapsedGameTime.TotalSeconds); drawRectangle.X = (int)location.X - halfDrawRectangleWidth;
            ...
            if (drawRectangle.Contains(mouse.X, mouse.Y))
            {
                ...
                {
                    leftButtonReleased = true;                    // if click finished on teddy, start collecting if target set
                    if (leftClickStarted)
                    {
                        if (targetSet)
                        {
        ...
        {
            targetSet = true;            // STUDENTS: set teddy velocity based on teddy center location and target
